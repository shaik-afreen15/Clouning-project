      ...
      signInWithEmailAndPassword(auth, email, password)
        .then((authUser) => {
          if (authUser) {
            ...
            navigate("/dashboard");
            window.dispatchEvent(new Event("recently-updated")); // refresh recently watched
          }
        })
    ...
    <div className="login">
      <div className="holder">
        <h1 className="text-white">
          {page ? languagesText[lang].loginTitle : languagesText[lang].registerTitle}
        </h1>
        <br />
        ...
        <form>
          ...
          <button className="btn btn-danger btn-block" type="submit" onClick={ctaClickHandler}>
        ...
        {isUserExist && <p className="text-danger">{languagesText[lang].userNotExist}</p>}
        {isEmailUsed && <p className="text-danger">{languagesText[lang].emailUsed}</p>}
